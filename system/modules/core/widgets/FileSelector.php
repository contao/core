...
class FileSelector extends \Widget
{
	...
	protected function renderFiletree($path, $intMargin, $mount=false, $blnProtected=false)
	{
		...
		for ($f=0, $c=count($folders); $f<$c; $f++)
		{
			...
			$return .= "\n    " . '<li class="'.$folderClass.'" onmouseover="Theme.hoverDiv(this, 1)" onmouseout="Theme.hoverDiv(this, 0)"'.((\Input::get('act') == 'select') ? ' onclick="Theme.toggleSelect(this)"' : '').'><div class="tl_left" style="padding-left:'.$intMargin.'px">';
		...
		if ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['files'] || $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strField]['eval']['filesOnly'])
		{
			...
			for ($h=0, $c=count($files); $h<$c; $h++)
			{
				...
				$return .= "\n    " . '<li class="tl_file" onmouseover="Theme.hoverDiv(this, 1)" onmouseout="Theme.hoverDiv(this, 0)"'.((\Input::get('act') == 'select') ? ' onclick="Theme.toggleSelect(this)"' : '').'><div class="tl_left" style="padding-left:'.($intMargin + $intSpacing).'px">';
