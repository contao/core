...
class TrblField extends Widget
{
	...
	public function generate()
	{
		...
		{
			$arrFields[] = sprintf('<input type="text" name="%s[%s]" id="ctrl_%s" class="tl_text_trbl%s" value="%s"%s onfocus="Backend.getScrollOffset();" />',
									$this->strName,
									$strKey,
									$this->strId.'_'.$strKey,
		...
		return sprintf('%s <select name="%s[unit]" class="tl_select_unit" onfocus="Backend.getScrollOffset();">%s</select>',
						implode(' ', $arrFields),
						$this->strName,
						implode('', $arrUnits));
	}
