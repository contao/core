...
<div class="tl_submit_container">
  ...
  <input type="file" name="'.$i.'" class="tl_upload_field" maxlength="'.$GLOBALS['TL_CONFIG']['maxFileSize'].'" onfocus="Backend.getScrollOffset();"><br>';
		...
		{
			$GLOBALS['TL_CSS'][] = TL_PLUGINS_URL .'plugins/fancyupload/css/fancyupload.css?'. FANCYUPLOAD .'|screen';
			$GLOBALS['TL_JAVASCRIPT'][] = TL_PLUGINS_URL .'plugins/fancyupload/js/fancyupload.js?'. FANCYUPLOAD;
...
<div class="tl_formbody_edit">
<input type="hidden" name="FORM_SUBMIT" value="'.$this->strTable.'_all">'.($blnIsError ? '
...
<input type="submit" name="saveNclose" id="saveNclose" class="tl_submit" accesskey="c" value="'.specialchars($GLOBALS['TL_LANG']['MSC']['saveNclose']).'">
	...
	protected function generateTree($path, $intMargin, $mount=false, $blnProtected=false, $arrClipboard=false)
	{
		...
		for ($h=0; $h<count($files); $h++)
		{
			...
			{
				$return .= '<a href="contao/popup.php?src='.base64_encode($currentEncoded).'" title="'.specialchars($GLOBALS['TL_LANG']['MSC']['view']).'" rel="lightbox[details '.$popupWidth.' '.$popupHeight.']">' . $this->generateImage($objFile->icon).'</a> '.utf8_convert_encoding(specialchars(basename($currentFile)), $GLOBALS['TL_CONFIG']['characterSet']).$thumbnail.'</div> <div class="tl_right">';
			}
