...
if(!class_exists('TCPDF', false)) {
	 ...
	 */ 
	define('PDF_PRODUCER','TCPDF 3.0.005 (http://www.tcpdf.org)');
			...
			switch($tag) {
				...
				case 'img': {
					if(isset($attr['src'])) {
						...
						$attr['width'] = round($attr['width'] / PDF_IMAGE_SCALE_RATIO); // TYPOlight patch
						$attr['height'] = round($attr['height'] / PDF_IMAGE_SCALE_RATIO); // TYPOlight patch
						$this->Ln(); // TYPOlight patch
				...
				case 'h6': {
					$this->Ln(); // TYPOlight patch
					$headsize = (4 - (substr($tag, 1) * 1.75)) * 2; // TYPOlight patch
					$currentFontSize = $this->FontSize;
					$this->tempfontsize = $this->FontSizePt;
					$this->SetFontSize($this->FontSizePt + $headsize);
					$this->setStyle('b', true);
					$this->lasth = $this->FontSize * K_CELL_HEIGHT_RATIO * 1.5; // TYPOlight patch
					break;
				}
			}
		}
